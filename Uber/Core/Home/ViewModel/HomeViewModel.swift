...
extension HomeViewModel {
    ...
    func cancelTripReqest() {
        ...
        tripService.updateTripRequest(
            id: tripRequest.id, update: updateTripRequest
        ) { success in
            if success {
    ...
    func addRiderRequestObserver() {
        guard let currentUser = currentUser, currentUser.accountType == .rider
        else { return }
    ...
    func requestTrip(rideType: RideType) {
        ...
        guard let pickupLocation = selectedPickupLocation else { return }
...
extension HomeViewModel {
    ...
    func rejectTripRequest(request: TripRequest) {
        ...
        tripService.updateTripRequest(id: request.id, update: updateTripRequest)
        { success in
            if success {
    ...
    func addDriverRequestsObserver() {
        guard let currentUser = currentUser, currentUser.accountType == .driver
        else { return }
        ...
        tripService.addTripRequestObserver(forDriver: currentUser.uid)
            ...
            .sink { completion in
                ...
                if request.state == .cancelled || request.state == .accepted {
                    ...
                    self.driverRequests = self.driverRequests
                        .appendIfNotContains(request)
                }
            }
            .store(in: &cancellables)
    }
    ...
    func updateTripTravelDetails() {
        guard let user = currentUser, user.accountType == .driver else {
            return
        }
        ...
        group.enter()
        getDestinationRoute(
            from: user.coordinates.toCLLocationCoordinate2D(),
            to: trip.pickupLocation.toCLLocationCoordinate2D()
        ) { route in
            travelDetails.travelTimeToPickup = Int(
        ...
        group.enter()
        getDestinationRoute(
            from: trip.pickupLocation.toCLLocationCoordinate2D(),
            to: trip.dropoffLocation.toCLLocationCoordinate2D()
        ) { route in
            travelDetails.travelTimeToDropoff = Int(
                route.expectedTravelTime / 60)
        ...
        group.notify(queue: .main) {
            ...
            self.tripService.updateTrip(id: trip.id, update: updateTrip) {
                success in
...
extension HomeViewModel {
    ...
    func getUberLocationFromCLLocation(
        _ location: CLLocation, completion: @escaping (UberLocation?) -> Void
    ) {
        CLGeocoder().reverseGeocodeLocation(location) { placemarks, error in
            if let error = error {
                debugPrint(
                    "Error reversing location \(error.localizedDescription)")
                completion(nil)
                return
            }
            ...
            let uberLocation = UberLocation(
                title: placemark.name!, address: placemark.formattedAddress,
                coordinate: UserCoordinates.from(
    ...
    func selectLocation(
        _ localSearch: MKLocalSearchCompletion, config: LocationResultViewConfig
    ) {
        locationSearch(forLocalSearchCompletion: localSearch) {
            response, error in
            if let error = error {
                debugPrint(
                    "Error searching for location: \(error.localizedDescription)"
                )
                return
            }
            ...
            let uberLocation = UberLocation(
                title: localSearch.title, address: localSearch.subtitle,
                coordinate: UserCoordinates.from(coordinate: coordinate))
            ...
            case .savedLocation(let viewModel):
                guard let uid = SupabaseManager.auth.currentUser?.id.uuidString
                else { return }
                ...
                if let currentUser = self.currentUser {
                    self.userService.updateUser(uid: currentUser.uid, updateUser: updateUser) { appUser in
    ...
    func computeRidePrice(forType type: RideType) -> Double {
        guard let dropoffCoordinate = selectedDropoffLocation?.coordinate else {
            return 0.0
        }
        guard let pickupCoordinate = selectedPickupLocation?.coordinate else {
            return 0.0
        }

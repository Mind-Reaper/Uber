...
class AuthViewModel: ObservableObject {
    ...
    func signIn(usingEmail email: String, password: String) {
        Task {
            do {
                let result = try await SupabaseManager.auth.signIn(
                ...
                debugPrint(
                    "Failed to sign in with error: \(error.localizedDescription)"
                )
            }
        }
    }
    ...
    func signUp(
        usingEmail email: String, password: String, firstname: String,
        lastname: String
    ) {
        Task {
            do {
                let result = try await SupabaseManager.auth.signUp(
                    email: email, password: password)
                ...
                debugPrint(
                    "Failed to sign up with error: \(error.localizedDescription)"
                )
    ...
    func changeAccountToDriver() {
        ...
        Task {
            ...
            do {
                ...
                let updateUser = UpdateUser(accountType: .driver)
                ...
                debugPrint(
                    "Error changing account type: \(error.localizedDescription)"
                )
            }
    ...
    func updateUserCoordinates() {
        ...
        Task {
            do {
                ...
                debugPrint(
                    "Error updating user coordinates: \(error.localizedDescription)"
                )
            }
        }
    }
